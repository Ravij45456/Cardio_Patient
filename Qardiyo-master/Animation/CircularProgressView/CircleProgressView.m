...
- (void)awakeFromNib {
    [super awakeFromNib]; // Added by VOG.
    self.progressLabel.numberOfLines = 0;
    self.progressLabel.lineBreakMode = NSLineBreakByWordWrapping;
    [self setupViews];
}
...
- (void)setupViews {
    ...
    // Sets the color right behind the wheel.
...
- (NSAttributedString *)formatProgressStringFromTimeInterval:(NSTimeInterval)interval {
    ...
    NSString *progressString = @"\n";//[self stringFromTimeInterval:interval shortDate:false];
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone){
        ...
        if (_status.length > 0) {
            ...
            [attributedString addAttributes:@{
                                              NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue-Bold" size:70]}
                                      range:NSMakeRange(0, progressString.length)];
        ...
        else
        {
            ...
            [attributedString addAttributes:@{
                                              NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue-Bold" size:36]}
                                      range:NSMakeRange(0, progressString.length)];
        }
    }
